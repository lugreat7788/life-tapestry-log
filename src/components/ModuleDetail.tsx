...
import { getDailyLog, toggleEntry, updateEntryNotes } from "@/lib/store";
...
export default function ModuleDetail({ moduleKey }: ModuleDetailProps) {
  ...
  const handleToggle = (itemId: string, points: number) => {
    const updated = toggleEntry(moduleKey, itemId, points);
    setLog({ ...updated });
  };

  const handleNotes = (itemId: string, notes: string) => {
    const updated = updateEntryNotes(itemId, moduleKey, notes);
    setLog({ ...updated });
  };
