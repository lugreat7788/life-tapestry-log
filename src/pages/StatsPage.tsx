...
import { getAllLogs, getWeekPoints, getStreakDays, getDailyLog } from "@/lib/store";
import { Flame, TrendingUp, Target, ChevronLeft, ChevronRight, X } from "lucide-react";
    ...
    <div className="px-4 pt-6 pb-4 max-w-lg mx-auto">
      ...
      <div className="bg-card rounded-xl shadow-card p-4 mb-6">
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  level > 0 ? LEVEL_COLORS[level] : "hover:bg-muted",
              ...
              >
                <span className={cn(
                  "font-medium",
                  level >= 3 ? "text-primary-foreground" : "text-foreground"
                )}>
                  {format(day, "d")}
                </span>
                {scores.total > 0 && (
                  <span className={cn(
                    ...
                    level >= 3 ? "text-primary-foreground/70" : "text-muted-foreground"
                  )}>
                    {scores.total}
                  </span>
                )}
        ...
        <div className="flex items-center justify-center gap-2 mt-3">
          ...
          {[1, 2, 3, 4].map((l) => (
            <div key={l} className={cn("w-3 h-3 rounded-sm", LEVEL_COLORS[l])} />
          ))}
              ...
              {selectedDetail.completedItems.length === 0 ? (
                ...
                <div className="space-y-2">
                  <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">
                    完成项目 ({selectedDetail.completedItems.length})
                  </h3>
