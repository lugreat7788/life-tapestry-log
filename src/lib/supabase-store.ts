...
export async function getScreenTimeRecord(userId: string, date?: Date): Promise<ScreenTimeRecord | null> {
  const dateStr = format(date || new Date(), "yyyy-MM-dd");
  const { data } = await supabase
    .from("screen_time_records" as any)
    .select("*")
    .eq("user_id", userId)
    .eq("date", dateStr)
    .maybeSingle();
...
export async function upsertScreenTimeRecord(
  userId: string,
  date: string,
  updates: Partial<{
    totalMinutes: number;
  ...
  const mapped: any = {};
  if (updates.totalMinutes !== undefined) mapped.total_minutes = updates.totalMinutes;
  if (updates.pickups !== undefined) mapped.pickups = updates.pickups;
  if (updates.categoryBreakdown !== undefined) mapped.category_breakdown = updates.categoryBreakdown;
  if (updates.screenshotUrl !== undefined) mapped.screenshot_url = updates.screenshotUrl;
  if (updates.notes !== undefined) mapped.notes = updates.notes;

  if (existing) {
    await supabase.from("screen_time_records" as any).update(mapped).eq("id", (existing as any).id);
  } else {
    await supabase.from("screen_time_records" as any).insert({
      user_id: userId,
      date,
      ...mapped,
    });
...
export async function getScreenTimeHistory(userId: string, days = 30): Promise<ScreenTimeRecord[]> {
  const { data } = await supabase
    .from("screen_time_records" as any)
    .select("*")
    .eq("user_id", userId)
